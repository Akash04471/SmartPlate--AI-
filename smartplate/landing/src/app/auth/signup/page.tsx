...
import { ArrowLeft, User, Mail, Lock, Target, ChevronRight, Check, Wifi, ShieldCheck, Activity } from "lucide-react";
import BiometricScan from "@/components/BiometricScan";
import { signup as apiSignup, upsertProfile } from "@/utils/api";
...
export default function SignupPage() {
  ...
  const handleFinalize = async () => {
    ...
    try {
      ...
      if (weightKg && heightCm && age) {
        ...
        const a = Number(age);
        const bmr = 10 * w + 6.25 * h - 5 * a + 5; // male default
        const tdee = bmr * 1.55; // moderately active default
  ...
  return (
    <main className="min-h-screen bg-base-dark flex items-center justify-center p-6 relative overflow-hidden selection:bg-white/10">
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[1000px] h-[1000px] bg-white/[0.01] rounded-full blur-[140px] pointer-events-none" />
      <BiometricScan active={isFocused} />
      
      <div className="absolute top-10 left-10 pointer-events-none gap-3 flex flex-col">
        <div className="flex items-center gap-3 text-white/20">
          <Wifi size={14} className="animate-pulse" />
          <span className="text-xs tracking-widest font-bold uppercase" style={{ fontFamily: 'var(--font-label)' }}>Secure Link</span>
        </div>
        <div className="flex items-center gap-3 text-white/20">
          <ShieldCheck size={14} />
          <span className="text-xs tracking-widest font-bold uppercase" style={{ fontFamily: 'var(--font-label)' }}>Encrypted</span>
        </div>
      </div>
      ...
      <AnimatePresence>
        {isCalibrating && (
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="fixed inset-0 z-[100] bg-base-dark flex flex-col items-center justify-center"
          >
            <div className="w-64 h-[1px] bg-white/5 mb-8 relative overflow-hidden">
              <motion.div 
                initial={{ width: 0 }}
                animate={{ width: "100%" }}
            ...
            <h2 className="text-4xl font-bold text-white tracking-tight" style={{ fontFamily: 'var(--font-display)' }}>Setting Up SmartPlate</h2>
            <p className="text-sm text-white/30 mt-4 tracking-wide font-medium" style={{ fontFamily: 'var(--font-label)' }}>Personalizing your nutrition plan...</p>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="max-w-3xl w-full relative z-10 transition-all duration-700">
        ...
        <div className="obsidian-card p-12 bg-base-dark/60 backdrop-blur-3xl border-white/5">
          <div className="flex justify-between items-start mb-14 border-b border-white/5 pb-8">
            <div className="flex-1">
              <span className="text-xs text-white/40 tracking-[0.2em] font-bold block mb-4 uppercase" style={{ fontFamily: 'var(--font-label)' }}>Step {step} of {totalSteps}</span>
              <h1 className="text-5xl font-bold text-white tracking-tight" style={{ fontFamily: 'var(--font-display)' }}>
            ...
            {step === 2 && (
              <div className="space-y-8">
                ...
                <div className="flex gap-6">
                  ...
                  <button onClick={handleStep2} className="flex-1 py-5 flex items-center justify-center gap-4 text-base bg-white text-black border border-white hover:bg-transparent hover:text-white transition-all font-semibold tracking-wide rounded-lg" style={{ fontFamily: 'var(--font-label)' }}>Continue <Activity size={20}/></button>
                </div>
              </div>
            )}
